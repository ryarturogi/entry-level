...
const JobCard = ({ job }) => {
  ...
  return (
    <section className="grid w-full grid-cols-12 gap-12 px-2">
      ...
      <div className="col-span-12 px-10 py-12 bg-white xl:col-span-4 rounded-2xl">
              ...
              <>
                <ul className="flex flex-col w-full mt-4 space-y-4">
                  {job.companyJobs.slice(0, 4).map((job) => (
                    <li key={job.id}>
                      <Link
                        className="flex items-center justify-between px-3.5 rounded-lg py-4 space-x-4 text-base font-light text-gray-800 border-2 border-primary-50"
                        href={`/job/${job.id}`}
                      >
                        ...
                        <div className="grid min-w-[40px] min-h-[40px] rounded place-content-center bg-primary-100">
                          <EyeIcon className="w-6 h-6 text-primary-800" />
                        </div>
                      </Link>
                    </li>
                  ))}
                ...
                {job.companyJobs.length > 4 && (
                  <div className="flex items-center justify-center w-full mt-4">
                    <Button
                      className="px-12 py-2 text-lg font-semibold text-white uppercase rounded-md bg-primary-500 hover:bg-primary-700"
